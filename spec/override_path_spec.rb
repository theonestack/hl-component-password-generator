...
describe 'compiled component password-generator' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/override_path.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/override_path/password-generator.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "PasswordSSMSecureParameter" do
      ...
      it "to have property Path" do
          expect(resource["Properties"]["Path"]).to eq({"Fn::Sub"=>"/${Identifier}/${EnvironmentName}/${MyPathSuffix}"})
      end
